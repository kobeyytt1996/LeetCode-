...
3. 字符串
   ...
   1. 递归（回溯）：题17 电话号码的字母组合（全排列，就想递归。也可以用队列，虽然时空复杂度没区别，但是个很好的思路）、题93. 复原 IP 地址（过程中为了便于操作，可以用vector存储最后再转为string）
...
6. 树：
      ...
      1. 二叉树：
            1. 遍历：
               ...
               2. 先序遍历：题100. 相同的树（写法可尽量简化）、题101. 对称二叉树（官解有BFS解法，看看即可）、题105、106. 从前序与中序遍历序列构造二叉树（递归方法细心即可。了解迭代算法即可，看官解）、题108. 将有序数组转换为二叉搜索树（中间节点选偏左或偏右均可，也可随机选择）、题112、113. 路径总和（BFS也可做，了解即可，提供思路，想从子节点回到根节点，可以用hashMap存每个节点和其父亲）、题129. 求根节点到叶节点数字之和（题目本身不难。是否应考察迭代：先思考递归写法，而递归函数的参数就代表迭代时栈里除了node要保留的值）
