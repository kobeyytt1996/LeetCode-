...
1. 数组
   ...
   2. 排序：题15 三数之和（易错点在于去除重复。关键部分还是写对快排）、题56. 合并区间（vector本身有重载operator<，不用提供functor）、**题164. 最大间距**（根据题目要求，线性时间的排序就想基数排序和桶排序，因桶排序并不真的需要完成排序，只用得到每个桶的最大最小值，因此表现更好一些。注意实际中logn的值也不会太大，因此快排完成复杂度也能接受。）
...
7. 数字：
   ...
   4. 遍历：
      1. 题59. 螺旋矩阵 II（还是按照上下左右四个边界进行迭代）、题118、119. 杨辉三角 （118：写法兼顾效率简洁，知道vector的最终大小可以提前resize，避免动态成长。**119：**用公式时要想到避免重复计算，即能得到横向递推）、168. Excel表列名称（十进制转二十六进制。但易错，不是0到26，而是1到26）
