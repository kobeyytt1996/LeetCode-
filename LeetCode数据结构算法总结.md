...
5. 链表：
   ...
   1. 单链表：
      1. 双指针：题19 删除链表的倒数第 N 个结点、题86. 分隔链表（即partition的单链表版本。由于链表的特性，这里可以保证稳定性）、题142. 环形链表 II（[官解](https://leetcode-cn.com/problems/linked-list-cycle-ii/solution/huan-xing-lian-biao-ii-by-leetcode-solution/)，要注意到slow走的距离就是环中节点数的倍数，然后不**需要再专门计算环中节点的个数**）、题143. 重排链表（栈可以做。但注意链表通常有节省空间复杂度的断开再重连的方式）
