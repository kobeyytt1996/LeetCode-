...
1. 数组
   ...
   7. 辅助数据结构：题41. 缺失的第一个正数（原地哈希，常见的数组当作哈希表问题，细心）、题49. 字母异位词分组（判断相同元素直接想到hashMap，关键在于不同的key和hash functor带来的效率差距。自己尝试重写了hash，但效果不好，最好尝试用STL提供的版本，官方解答的两种方法效率更高。**复杂度较高的问题，做排序通常能优化。**）、题71.简化路径、题84. 柱状图中最大的矩形（单调栈。核心在于找到一个元素对应的两边比它小的元素）、题**128. 最长连续序列**（不要想着一下子找到O(n)的解法，要一步一步优化）、**题149. 直线上最多的点数**（关键还是题意的理解。[重要技巧](https://leetcode-cn.com/problems/max-points-on-a-line/solution/gong-shui-san-xie-liang-chong-mei-ju-zhi-u44s/)：计算斜率等值时为浮点数，可能精度有问题，故能存整数最好存整数，两个整数还可以取得最大公约数，为了当key还可以拼成string）
   8. 贪心：题45. 跳跃游戏 II、
